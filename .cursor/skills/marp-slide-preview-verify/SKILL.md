...
Marp CLI v4 の **`--server`** では、入力ディレクトリは **`slides` を positional で渡さず `-I slides`** と書く（`... slides` だけだと Usage のみでサーバーが立たないことがある）。
...
| ステップ | 操作 |
|---------|------|
| 1 | `npx @marp-team/marp-cli@latest --no-stdin --server --theme-set themes/tech.css -I slides` をバックグラウンドで実行（リポジトリルート） |
